...
//       .map((q) => q.options.$values.map((opt) => opt.label).join(', '))
//       .join('\n\n');
...
export class AdminSurveysComponent implements OnInit {
  ...
  editSurvey(surveyId: number): void {
    ...
    this.editTitle = survey.title;
    this.editDueDate = new Date(survey.dueDate);
    this.editQuestionsText = survey.questions.$values.map((q: any) => q.heading).join('\n\n');
    this.editOptionsText = survey.questions.$values.map((q: any) => q.options.$values.map((opt: any) => opt.label).join(', ')).join('\n\n');
