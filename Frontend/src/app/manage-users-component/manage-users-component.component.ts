...
export class ManageUsersComponent implements OnInit {
    ...
    constructor(private userService: UserService, private surveyService: SurveyService, private router: Router, private cd: ChangeDetectorRef) {
        this.newUsername = '';
        this.newPassword = '';
        this.newGroupName = '';
    ...
    createGroup(): void {
        ...
        if (this.newGroupName && this.newGroupName.trim() !== '') {
            this.userService.createGroup(this.newGroupName).subscribe(() => {
                ...
                this.errorMessage = 'There was an issue creating the group. Please try again later.';
    ...
    editUser(user: any): void {
        ...
        if (this.editingUser && this.editingUser.userGroups && this.editingUser.userGroups.$values) {
            ...
            this.editingUser.userGroups.$values.forEach(userGroup => {
                const matchingGroup = this.editingGroups.find(g => g.id === userGroup.groupId);
                if (matchingGroup) {
                    matchingGroup.selected = true;
                }
            });
    ...
    updateUser(event: Event): void {
        event.preventDefault();
        if (this.editingUser) {
            const selectedGroups = this.editingGroups.filter(group => group.selected).map(group => group.id);
            this.editingUser.groupIds = selectedGroups;
            console.log(this.editingUser.groupIds);
            this.userService.updateUser(this.editingUser).subscribe(() => {
    ...
    createSurvey(): void {
        ...
        if (!this.newSurveyTitle.trim() || !this.newSurveyDueDate || !this.newSurveyQuestions.trim()) {
            this.errorMessage = 'Please fill out all the fields.';
            return;
        }

        // Extract individual questions from the textarea content
        const questionsArray = this.newSurveyQuestions.split('\n').filter(question => question.trim() !== '');
        ...
        this.surveyService.createSurvey(surveyPayload).subscribe(response => {
