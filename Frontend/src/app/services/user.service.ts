...
export class UserService {
  ...
  addUser(username: string, password: string, groupId?: number): Observable<void> {
    const userPayload = {
      username: username,
  ...
  updateUser(user: User): Observable<void> {
    return this.http.put<void>(`${this.baseUrl}/user/updateuser/${user.id}`, user);
  }

  // Create a new user
  createUser(payload: { username: string, password: string, groupIds: number[] }): Observable<any> {
  ...
  createGroup(groupName: string): Observable<void> {
    return this.http.post<void>(`${this.baseUrl}/group/creategroup`, { groupName: groupName });
  }
  ...
  getGroups(): Observable<Group[]> {
    return this.http.get<{ $id: string; $values: Group[] }>(`${this.baseUrl}/group/getgroups`)
      .pipe(
        map(response => response.$values.map(group => {
          const userGroups = group.userGroups || { $id: "", $values: [] };
          const surveysAssigned = group.surveysAssigned || { $id: "", $values: [] };
  ...
  addUserToGroups(payload: { userId: number, groupIds: number[] }): Observable<void> {
    return this.http.post<void>(`${this.baseUrl}/user/addusertogroups`, payload);
  }
